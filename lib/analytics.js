...
// Consent is stored in localStorage under the key: 'adons-consent-preferences'
// This is unified with ConsentProvider to ensure consistent consent management

const CONSENT_KEY = 'adons-consent-preferences'
let _gtmId = ''
...
function _consentGiven() {
  try {
    ...
    const preferences = localStorage.getItem(CONSENT_KEY)
    if (!preferences) return false
    const parsed = JSON.parse(preferences)
    // Check if analytics consent is specifically granted
...
export function setConsent(granted = true) {
  try {
    ...
    // Update consent preferences to match ConsentProvider format (no marketing)
    const preferences = {
      analytics: granted,
      functional: true,
      necessary: true
